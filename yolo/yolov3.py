...
def parse_cfg(config_file):
    ...
    for line in file:
        ...
        if line[0] == "[":
            if (
                len(element_dict) != 0
            ):  # appending the dict stored on previous iteration
            ...
            val = line.split("=")
            element_dict[val[0].rstrip()] = val[
                1
            ].lstrip()  # removing spaces on left and right side
...
def prediction(x, inp_dim, anchors, num_classes, CUDA=False):
    ...
    prediction[:, :, 2:4] = (
        torch.exp(prediction[:, :, 2:4]) * anchors
    )  # width and height
...
def write_results(prediction, confidence, num_classes, nms_conf=0.4):
    ...
    for ind in range(batch_size):
        ...
        non_zero_ind = torch.nonzero(
            image_pred[:, 4]
        )  # non_zero_ind will give the indexes
...
try:
    imlist = [
        os.path.join(os.path.realpath("."), input_dir, img)
...
for batch in im_batches:
    ...
    prediction = write_results(
        prediction, confidence=0.5, num_classes=80, nms_conf=nms_thesh
    )
    ...
    if not write:
        ...
        output = torch.cat(
            (output, prediction)
        )  # concating predictions from each batch
    i += 1
...
det_names = pd.Series(imlist).apply(
    lambda x: "{}/det_{}".format(output_dir, x.split("/")[-1])
)
...
# see the detection results in ./kaggle/working/output/

# img = cv2.imread("./kaggle/working/output/det_dog-cycle-car.png")
# img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
# plt.figure(figsize=(20, 10))
# plt.imshow(img)
# plt.show()

# img = cv2.imread('./kaggle/working/output/det_office.jpg')
# img = cv2.cvtColor(img,cv2.COLOR_BGR2RGB)
# plt.figure(figsize=(20,10))
# plt.imshow(img)